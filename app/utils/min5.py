"""【已废弃】baostock 5 分钟线抓取 → `data/min5/daily/`（未注册、无读取方）。
...
# 原因：日期区间硬编码、无视 DATA_JOB_* 环境变量，且写入的 min5/daily
...
def _fetch_minute(stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
    """用 baostock 拉单只股票的 5 分钟 K 线（前复权），逐行迭代结果集拼表。
    """
...
def main():
    """作业入口：登录 baostock，按 stock_basic 清单逐只拉 5 分钟线并汇总落盘。

    **窗口日期是写死的常量**（历史遗留脚本），stock_basic 为空时跳过。
    """
    stock_list = get_stock_codes()
    if not stock_list:
        print("[min5] stock_basic.parquet is empty, skip.")
        return
    ...
    try:
        ...
        for ts_code in stock_list:
            df = _fetch_minute(_to_bs_code(ts_code), "2025-04-01", "2025-05-29")
        ...
        if frames:
            ...
            total_saved = save_partitioned_parquet(combined, "trade_date", "min5/daily")
            print(f"[min5] 完成，写入={total_saved}")
    finally:
        bs.logout()
